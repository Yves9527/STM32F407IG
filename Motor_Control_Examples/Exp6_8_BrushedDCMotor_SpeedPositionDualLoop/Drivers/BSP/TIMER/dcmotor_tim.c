...
#define MOTOR_MAX_TARGET_SPEED        150.0f  /* 位置环输出限幅（目标速度最大值） */
...
/**
 * @brief       基本定时器TIMX定时中断初始化函数 编码器采样
 * @note
...
#endif
        motor_pwm_set((int32_t)g_motor_data.motor_pwm);         /* 设置占空比（电机转速） */
    }
    else
    {
        motor_pwm_set(0);                                       /* 停止输出 */
    }
}
/**
 * @brief       定时器更新中断回调函数
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
     ...
     else if (htim->Instance == TIM6)       
     {
        ...
        speed_computer(Encode_now, MOTOR_SPEED_CALC_PERIOD_MS);          /* 中位平均值滤除编码器抖动数据，5ms计算一次速度 */
        g_motor_data.location = (float)Encode_now;                      /* 获取当前编码器总计数值，用于位置闭环控制 */

        if (++val >= SMAPLSE_PID_SPEED)                                 /* 进行一次pid计算 */
        {
            val = 0;
            motor_dual_loop_control_step();                             /* 执行双环PID闭环控制 */
        }
     }
}
