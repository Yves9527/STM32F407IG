...
void atim_timx_pwm_chy_init(uint16_t arr, uint16_t psc)
{
    ...
    HAL_TIM_PWM_ConfigChannel(&g_atimx_pwm_chy_handle, &g_atimx_oc_pwm_chy_handle, ATIM_TIMX_PWM_CH3); /* 配置TIMx通道y */
    ...
    HAL_TIM_PWM_Start(&g_atimx_pwm_chy_handle, ATIM_TIMX_PWM_CH3);     /* 开启对应PWM通道 */
}
